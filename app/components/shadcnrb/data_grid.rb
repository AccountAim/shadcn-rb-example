...
# data_grid/component_controller.js; call it before `g.body`. With
# `columns:` a bare `g.header fixed: true` renders the heads itself.
# `selectable: true` makes it an ARIA grid: click or arrow keys move the
# selected cell, kept in view by the same controller, which reports
# `data-grid:select / activate / commit / cancel / leave` for an app to
# build editing on; `editor:` names a Stimulus controller the grid also
# calls by method name. `g.cell ..., selectable: false` keeps a cell out.
...
module Shadcnrb
  class DataGrid < Component
    ...
    def data_grid(columns: nil, cards: false, selectable: false, editor: nil, size: :default,
      **opts, &block)
      ...
      wrapper_class = Shadcnrb::TailwindMerge.call(
        @fixed ? style.fixed_container : style.container, opts.delete(:class)
      )
      base = @fixed ? style.fixed_base : style.base
      root_class = [ base, size_class, (style.selectable if selectable) ]
      opts[:class] = Shadcnrb::TailwindMerge.call(*root_class)
      ...
      if selectable
        ...
        opts[:data][:action] = merge_action(opts[:data], "keydown->#{CONTROLLER}#navigate",
          "click->#{CONTROLLER}#select", "dblclick->#{CONTROLLER}#activate",
          "focus->#{CONTROLLER}#enter", "focusin->#{CONTROLLER}#focused",
          "focusout->#{CONTROLLER}#blurred")
      end

      caption = @sections[:caption]
      opts[:aria] = (opts[:aria] || {}).merge(describedby: @caption_id) if caption
      parts = @sections.values_at(:header, :body, :footer).compact
      parts << tooltip_template if @columns.any? { |c| truncates?(c) }
      table = content_tag(:div, safe_join(parts), **opts)

      controllers = [ CONTROLLER, editor ].compact.join(" ")
      container_data = { slot: "data-grid-container", controller: controllers }
      container_data[:"#{CONTROLLER}-editor-value"] = editor if editor
      wrapper = content_tag(:div, table, class: wrapper_class, data: container_data,
        style: "font-size: #{card_breakpoint(cards)}")
      caption ? safe_join([ wrapper, caption ]) : wrapper
    ensure
      @sections = nil
    end
    ...
    # Rendered after the wrapper so a border doesn't enclose it;
    # `aria-describedby` on the grid keeps it announced.
    def caption(name = nil, scope: nil, **opts, &block)
      ...
      @caption_id = opts[:id] ||= "data-grid-caption-#{SecureRandom.hex(4)}"
      section(:caption, content_tag(:p, **opts) { block ? capture(&block) : name.to_s })
    end
