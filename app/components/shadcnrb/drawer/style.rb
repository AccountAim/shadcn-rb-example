...
class Shadcnrb::Drawer::Style
  ...
  # plus the free side set to `auto` on the panel undo the UA's `[popover]`
  # box (`inset: 0; margin: auto; fit-content sizing; border; padding; CanvasText`).
  def backdrop
    <<~CLASSES.squish
  ...
  def sides
    {
      ...
      top:    "inset-x-0 top-0 bottom-auto h-auto max-h-[80vh] mb-24 rounded-b-lg border-b -translate-y-full data-[state=open]:translate-y-0",
      bottom: "inset-x-0 bottom-0 top-auto h-auto max-h-[80vh] mt-24 rounded-t-lg border-t translate-y-full data-[state=open]:translate-y-0"
    }
  end
