...
module DocsHelper
  ...
  def source_block(path, only: nil)
    ...
    tag.div(class: "space-y-1") do
      tag.p(path, class: "font-mono text-xs text-muted-foreground") + sui.codeblock(code, syntax: syntax_for(code))
    end
  end
...
def api_table(rows:, headers: %w[Method Args Default Description])
    content_tag(:div, class: "overflow-hidden rounded-lg border") do
      sui.table do |t|
        safe_join([
          t.header { t.row { safe_join(headers.map { |h| t.head(h) }) } },
          t.body   { safe_join(rows.map { |r| t.row { safe_join(r.map { |c| t.cell(c.to_s.html_safe) }) } }) }
        ])
      end
    end
  end
